...
import com.koisv.dkm.discord.data.GuildData.RepeatType.*
...
object MusicHandler {
    ...
    suspend fun guildControl(id: Snowflake, type: UpdateType, data: List<String?> = listOf(null,null)) {
        val guild = dBotInstance.getGuildOrNull(id)?.asGuild()
        if (guild != null) {
            ...
            if (!dGDataList.contains(findGuild)) dGDataList.add(findGuild)
        }
    }
    ...
    class Event: AudioEventAdapter() {
        override fun onTrackStart(player: AudioPlayer?, track: AudioTrack?) {
            ...
            val findGuild = dGDataList.find { it.player == player } ?: return
