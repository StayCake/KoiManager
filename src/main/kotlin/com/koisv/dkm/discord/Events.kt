...
import com.koisv.dkm.discord.MusicHandler.QueryType
import com.koisv.dkm.discord.MusicHandler.UpdateType
import com.koisv.dkm.discord.commands.AdminCommand
import com.koisv.dkm.discord.commands.MusicCommand
import com.koisv.dkm.discord.commands.UtilCommand
import com.koisv.dkm.discord.data.Bot
import com.koisv.dkm.discord.data.Convert.getEnglishNumber
import com.koisv.dkm.discord.data.Convert.getTimeStamp
...
object Events {
    ...
    suspend fun handle(e: Event, botData: Bot) {
        when (e) {
            ...
            is InteractionCreateEvent -> {
                when (e) {
                    ...
                    is GuildChatInputCommandInteractionCreateEvent -> {
                        val guildData = dGDataList.find { it.id == e.interaction.guildId }
                        if (guildData == null) {
                            dGDataList.add(GuildData(e.interaction.guild.asGuild().name, e.interaction.guildId))
                        } else if (guildData.name != e.interaction.guild.asGuild().name) {
                            guildData.name = e.interaction.guild.asGuild().name
                        }
                        kordLogger.info ("[${
                                ...
                                e.interaction.command.rootName
                            }")
                        commandExecute(e, botData)
                    }
            ...
            is GuildCreateEvent -> {
                kordLogger.info("[+] ${e.guild.name}")
                val guildData = dGDataList.find { it.id == e.guild.id }
                if (guildData == null) {
                    dGDataList.add(GuildData(e.guild.name, e.guild.id))
                } else if (guildData.name != e.guild.name) {
                    guildData.name = e.guild.name
                }
            }
            is GuildDeleteEvent -> {
                kordLogger.info("[-] ${e.guild?.name}")
                dGDataList.removeIf { it.id == e.guild?.id }
            }
            is ReadyEvent -> {
                kordLogger.info("Logged On ${dBotInstance.getSelf().username} | ${dBotInstance.guilds.count()} Guilds")
                dBotUptime = Clock.System.now()
                dBotInstance.editPresence {
                    playing(botData.presence)
    ...
    suspend fun buttonInteract(event: ButtonInteractionCreateEvent) {
        ...
        when (ident[0]) {
            ...
            "MusicSearch" -> {
                ...
                val findGuild = dGDataList.find { it.id == guild.id } ?: GuildData(guild.name, guild.id)
                ...
                if (player.playingTrack != null) {
                    ...
                    findGuild.connection =
                        VoiceConnection(event.gateway, dBotInstance.selfId, channel.id, channel.guildId)
                        {
            ...
            "Playlist" -> {
                val findGuild = dGDataList.find { it.id == button.message.getGuild().id }
    ...
    suspend fun respondNonDM(it: DeferredMessageInteractionResponseBehavior) {
        return run { it.respond { content = "개인 DM에선 사용할 수 없습니다." } }
    }

    suspend fun respondError(it: GuildChatInputCommandInteractionCreateEvent) {
        return run { it.interaction.respondEphemeral { content = "명령 실행에 문제가 있습니다." } }
    }

    private suspend fun commandExecute(event: GuildChatInputCommandInteractionCreateEvent, botData: Bot) {
        ...
        when (event.interaction.command.rootName) {
            "반복" -> MusicCommand.repeat(
                event, subC1?.let { listOf(it.name) } ?: return respondError(event))
            ...
            "bmu" -> AdminCommand.manageBot(event, botData, subC2, subC1?.name)
        }
    }
}
