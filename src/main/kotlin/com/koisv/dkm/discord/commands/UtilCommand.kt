        ...
        val respond =
            if (
                event.interaction.channel.asChannelOf<TextChannel>()
                    .getEffectivePermissions(dBotInstance.getSelf().asMember(event.interaction.guildId).id)
                ...
                if (request.messages.size > 1)
                    dBotInstance.rest.channel.bulkDelete(event.interaction.channelId, request,"/청소 명령어 사용")
