...
@OptIn(KordVoice::class, ExperimentalEncodingApi::class)
object MusicCommand {
    suspend fun play(event: GuildChatInputCommandInteractionCreateEvent) {
        ...
        val state = event.interaction.user.getVoiceStateOrNull()
            ?: return run { reply.respond { content = "음성방에 입장해 주세요!" } }
        val channel = state.getChannelOrNull()
            ?: return run { reply.respond { content = "채널 로딩 오류!" } }

        if (findGuild !in dGDataList) dGDataList.add(findGuild)
        
        if (link != null) {
            val isLink = link.startsWith("http://") || link.startsWith("https://")
            
            val type = when {
                isLink && link.contains("playlist?list=") -> MusicHandler.QueryType.Playlist
                isLink && link.contains("watch?v=") -> MusicHandler.QueryType.Single
                isLink -> MusicHandler.QueryType.Single
            ...
            if (current.isEmpty()) findGuild.voiceLast = reply.respond { content = respond }.message
                ...
                if (player.playingTrack != null) findGuild.trackList.addAll(current)
                    ...
                    findGuild.connection =
                        channel.connect { audioProvider { player.provide()?.data?.let { AudioFrame(it) } } }
            ...
            if (findGuild.trackList.isNotEmpty()) {
                if (player.playingTrack != null)
                    return run { reply.respond { content = "이미 재생중입니다!" } }
            ...
            } else reply.respond { content = "재생목록이 텅 빈건가요..." }
        }
    }
    ...
    suspend fun stop(event: GuildChatInputCommandInteractionCreateEvent) {
        ...
        val guild = event.interaction.guild.asGuildOrNull() ?: throw NullPointerException("Guild Not Found")
        val findGuild = dGDataList.find { it.id == guild.id } ?: GuildData(guild.name, guild.id)
        MusicHandler.guildControl(guild.id, MusicHandler.UpdateType.Remove)
    ...
    suspend fun playlist(event: GuildChatInputCommandInteractionCreateEvent) {
        ...
        val findGuild = dGDataList.find { it.id == guild.id } ?: throw NullPointerException("Guild Not Found")
    ...
    suspend fun volume(event: GuildChatInputCommandInteractionCreateEvent) {
        val reply = event.interaction.deferPublicResponse()
        val guild = event.interaction.guild.asGuild()
        val findGuild = dGDataList.find { it.id == guild.id } ?: return Events.respondNonDM(reply)
